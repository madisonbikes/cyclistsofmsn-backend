import { configuration, overrideConfigurationForTests } from "../config";
...
export const setupSuite = (options: Partial<SuiteOptions> = {}): void => {
  ...
  beforeAll(async () => {
    ...
    if (withDatabase) {
      ...
      overrideConfigurationForTests({ mongodbUri });
...
const initializeSuite = async (withMutableTestResources: boolean) => {
  ...
  if (withMutableTestResources) {
    ...
    overrideConfigurationForTests({ photosDir: mutablePhotosDir });
  }
};
